...
class LlavaVid(lmms):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Convert relative path to absolute path if it's a local path
        # Check if it's a relative path (starts with ./ or ../) or contains / but not a HuggingFace repo format
        is_local_path = (
            os.path.exists(pretrained) or 
            pretrained.startswith('./') or 
            pretrained.startswith('../') or 
            (os.sep in pretrained and not pretrained.startswith('http') and '/' not in pretrained.split(os.sep)[0])
        )
        
        if is_local_path and not os.path.isabs(pretrained):
            # Convert relative path to absolute path
            # Try to resolve from current working directory first
            abs_path = os.path.abspath(pretrained)
            if os.path.exists(abs_path):
                pretrained = abs_path
            else:
                # If not found, try resolving from common project root locations
                # This handles cases where script is run from different directories
                possible_roots = [
                    os.path.join(os.path.dirname(__file__), '../../..'),  # From evaluation/llava_vid.py
                    os.getcwd(),
                ]
                for root in possible_roots:
                    test_path = os.path.abspath(os.path.join(root, pretrained))
                    if os.path.exists(test_path):
                        pretrained = test_path
                        break
    ...
    def generate_until(self, requests) -> List[str]:
        ...
        if self.rank == 0:
            ...
            if total_videos > 0:
                ...
                eval_logger.info(f"Video availability check: {total_videos - missing_count}/{total_videos} videos available ({100 - missing_percentage:.2f}%)")
                eval_logger.info(f"Missing videos: {missing_count}/{total_videos} ({missing_percentage:.2f}%)")
                ...
                if missing_percentage > 50.0:
                    ...
                    eval_logger.warning(f"Warning: {missing_count} videos are missing but continuing evaluation (missing percentage: {missing_percentage:.2f}% <= 50%)")
