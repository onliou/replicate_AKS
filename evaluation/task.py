...
class ConfigurableMessagesTask(ConfigurableTask):
    """A task class for chat-based models that use message-based prompts.
    
    ConfigurableMessagesTask extends ConfigurableTask to support message-based
    chat interfaces. The key difference is that it uses `doc_to_messages` instead
    of `doc_to_visual` and `doc_to_text` separately, allowing for structured
    message formats with roles (user, assistant, system) and content types
    (text, image, video, audio).
    """
    
    def __init__(self, data_dir=None, cache_dir=None, download_mode=None, config=None, model_name=None):
        """Initialize ConfigurableMessagesTask.
        
        Args:
            data_dir: Directory containing task data files
            cache_dir: Directory for caching datasets
            download_mode: How to handle dataset downloads
    ...
    def doc_to_messages(self, doc: dict) -> Union[int, str, list]:
        """Convert a document to a message format for chat-based models.
        
        This method handles three scenarios:
        1. If `doc_to_messages` is callable: use it directly
        2. If `doc_to_messages` is None but `doc_to_visual` or `doc_to_text` exist:
           automatically construct messages from visual and text content
        3. Otherwise: return the configured `doc_to_messages` value
        
        Args:
            doc: Document dictionary from the dataset
            
        Returns:
            Messages in chat format (list of dicts with 'role' and 'content' keys)
        """
        # Case 1: doc_to_messages is a callable function
        if callable(self.config.doc_to_messages):
            # Check if function accepts lmms_eval_specific_kwargs
            if self.lmms_eval_specific_kwargs is not None and len(inspect.signature(self.config.doc_to_messages).parameters) == 2:
                return self.config.doc_to_messages(doc, self.lmms_eval_specific_kwargs)
            else:
                return self.config.doc_to_messages(doc)
        
        # Case 2: Auto-generate messages from doc_to_visual and doc_to_text
            ...
            def auto_doc_to_messages(doc):
                ...
                - Single user message with content array
                - Content can include: images, audio, video, and text
                """
                # Get visual content (images, videos, etc.)
                ...
                for visual in visuals:
                    if isinstance(visual, PIL_Image.Image):
                        # PIL Image object - convert to message format
                        content.append({"type": "image", "url": visual})
                    elif isinstance(visual, dict):
                        # Dictionary format - assume audio
                        content.append({"type": "audio", "url": visual})
                    elif isinstance(visual, str):
                        # String format - assume video path/URL
        ...
        # Case 3: Return configured doc_to_messages value directly
        else:
            return self.config.doc_to_messages

    def construct_requests(self, doc_id: int, ctx: str, **kwargs) -> Union[List[Instance], Instance]:
        """Construct request instances for message-based tasks.
        
        Overrides parent method to use `doc_to_messages` instead of `doc_to_visual`.
        This is the key difference between ConfigurableTask and ConfigurableMessagesTask.
        
        Args:
            doc_id: Document index in the dataset
            ctx: Context string (may not be used for message-based tasks)
            **kwargs: Additional metadata (must include 'metadata' with 'split' key)
            
        Returns:
            Instance object for message-based generation
            
        Raises:
            AssertionError: If OUTPUT_TYPE is not "generate_until"
        """
        split = kwargs.get("metadata").get("split")
        
        # Message-based tasks currently only support generation
        assert self.OUTPUT_TYPE == "generate_until", "Currently messages is used for generation only"
        
        # Create arguments tuple:
        # - ctx: context string
        # - self.doc_to_messages: function to convert doc to messages
        # - generation_kwargs: deep copy of generation parameters
        # - doc_id: document index
        # - task name: for tracking
        # - split: dataset split name
        arguments = (
            ctx,
            self.doc_to_messages,  # Use doc_to_messages instead of doc_to_visual
    ...
    def __repr__(self):
        ...
        return (
            f"ConfigurableMessagesTask("
            f"task_name={getattr(self.config, 'task', None)},"
